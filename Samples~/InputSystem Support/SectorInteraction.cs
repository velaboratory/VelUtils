...
namespace unityutilities
{
	...
	public class SectorInteraction : IInputInteraction<Vector2>
	{
		...
		{
			bool isActuated = context.ControlIsActuated(pressPointOrDefault);
			...
			bool isValidDirection = IsValidDirection(ref context);
		...
		bool IsValidDirection(ref InputInteractionContext context)
		{
			...
			Directions nearestDirection = GetNearestDirection(cardinal);
...
#pragma warning restore IDE0051 // Remove unused private members
	...
	class SectorInteractionEditor : InputParameterEditor<SectorInteraction>
	{
		...
		public override void OnGUI()
		{
			...
			bool useDefaultValue = target.pressPoint < 0f;

			EditorGUILayout.BeginHorizontal();

			EditorGUI.BeginDisabledGroup(useDefaultValue);

			float newPressPoint = EditorGUILayout.Slider(m_PressPointLabel, target.pressPointOrDefault, 0f, 1f, GUILayout.ExpandWidth(false));
			if (!useDefaultValue)
			{
			...
			bool newUseDefault = GUILayout.Toggle(useDefaultValue, m_DefaultToggleLabel, GUILayout.ExpandWidth(false));
